                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class='breadcrumb-header'>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">SSNIT: {{ $pension->pension_tier }}
                            </li>
                        </ol>
                    </nav>
                </div>
                        ...
                        <tbody>
                            @foreach ($employees as $index => $employee)
                                <tr>
                                    ...
                                    <td>{{ $employee->last_name }} {{ $employee->middle_name }}
                                        {{ $employee->first_name }}</td>
                                    <td></td>
                                    <td>GHc {{ number_format($employee->salary, 2) }}</td>
                                    <td>GHc {{ number_format(($pension->percentage / 100) * $employee->salary, 2) }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
